...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_flavors_category" AS ENUM('classic', 'specialty', 'filled', 'seasonal');
  CREATE TYPE "public"."enum_orders_pickup_type" AS ENUM('same-day', 'next-day');
  CREATE TYPE "public"."enum_orders_status" AS ENUM('pending', 'paid', 'ready', 'picked-up', 'cancelled', 'refunded');
  ...
  CREATE TYPE "public"."enum_hours_schedule_day" AS ENUM('sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday');
  ...
  CREATE TABLE "flavors" (
  	...
  	"category" "enum_flavors_category" DEFAULT 'classic' NOT NULL,
  	"description" varchar,
  	"image_id" integer,
  	"price_cents" numeric NOT NULL,
  	"square_catalog_id" varchar,
  	"is_available_today" boolean DEFAULT false,
  	"is_on_rotation" boolean DEFAULT true,
  ...
  CREATE TABLE "orders_items" (
  	...
  	"flavor_name_snapshot" varchar,
  	"quantity" numeric DEFAULT 1 NOT NULL,
  	"price_at_order_cents" numeric NOT NULL
  );
  ...
  CREATE TABLE "orders" (
  	...
  	"pickup_type" "enum_orders_pickup_type" NOT NULL,
  ...
  CREATE TABLE "hours_schedule" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  	"day" "enum_hours_schedule_day" NOT NULL,
  ...
  CREATE TABLE "hours" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"sold_out_message" varchar DEFAULT 'We sell out daily — come early or message ahead to reserve!',
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  ...
  CREATE TABLE "location" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"venue_name" varchar DEFAULT 'Advance Auto Parts',
  	"address" varchar DEFAULT '2100 S Douglas Hwy' NOT NULL,
  	"city" varchar DEFAULT 'Gillette' NOT NULL,
  	"state" varchar DEFAULT 'WY' NOT NULL,
  	"zip" varchar DEFAULT '82718' NOT NULL,
  ...
  CREATE TABLE "site_settings" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"logo_id" integer,
  	"hero_tagline" varchar DEFAULT 'Fresh handmade donuts daily',
  	"hero_subtitle" varchar DEFAULT 'Warm, fluffy donuts made fresh every morning — classic favorites, filled donuts, and rotating specialty flavors.',
  	"primary_color" varchar DEFAULT '#f472b6',
  	"accent_color" varchar DEFAULT '#fbbf24',
  	"ordering_enabled" boolean DEFAULT false,
  	"same_day_cutoff_minutes_before_close" numeric DEFAULT 30,
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  ...
  ALTER TABLE "flavors" ADD CONSTRAINT "flavors_image_id_media_id_fk" FOREIGN KEY ("image_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "orders_items" ADD CONSTRAINT "orders_items_flavor_id_flavors_id_fk" FOREIGN KEY ("flavor_id") REFERENCES "public"."flavors"("id") ON DELETE set null ON UPDATE no action;
  ...
  ALTER TABLE "hours_schedule" ADD CONSTRAINT "hours_schedule_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."hours"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "flavors_image_idx" ON "flavors" USING btree ("image_id");
  CREATE INDEX "flavors_slug_idx" ON "flavors" USING btree ("slug");
  ...
  CREATE INDEX "orders_items_flavor_idx" ON "orders_items" USING btree ("flavor_id");
  CREATE UNIQUE INDEX "orders_order_number_idx" ON "orders" USING btree ("order_number");
  CREATE INDEX "orders_square_payment_id_idx" ON "orders" USING btree ("square_payment_id");
  ...
  CREATE INDEX "hours_schedule_order_idx" ON "hours_schedule" USING btree ("_order");
  CREATE INDEX "hours_schedule_parent_id_idx" ON "hours_schedule" USING btree ("_parent_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "hours_schedule" CASCADE;
  ...
  DROP TYPE "public"."enum_flavors_category";
  DROP TYPE "public"."enum_orders_pickup_type";
  ...
  DROP TYPE "public"."enum_hours_schedule_day";`)
}
